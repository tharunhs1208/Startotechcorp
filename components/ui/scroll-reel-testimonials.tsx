...
const QUOTE_CLASSES =
  "m-0 text-lg font-medium leading-[1.3] tracking-[-0.02em] text-[#111111] dark:text-foreground sm:text-[22px]";
const AUTHOR_CLASSES =
  "m-0 text-sm font-medium leading-[1.3] text-[#666666] dark:text-muted-foreground";
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-black/10 dark:border-border bg-gradient-to-b from-black/[0.03] to-white dark:from-secondary dark:to-card blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(255,255,255,1)] dark:shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.06)]"
...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    <div
      className="relative shrink-0 overflow-hidden rounded-xl bg-black/5 dark:bg-muted dark:ring-1 dark:ring-white/10"
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-2xl border border-black/[0.08] bg-white dark:bg-muted shadow-[inset_0_2px_0_rgba(255,255,255,1),0_10px_30px_rgba(0,0,0,0.04)] outline-none focus-visible:ring-2 focus-visible:ring-black md:min-h-[320px] md:flex-row",
        className
      )}
    >
      {/* Reel section */}
      <div
        aria-hidden="true"
        className="relative h-56 w-full shrink-0 self-stretch overflow-hidden md:h-auto md:w-[380px] bg-[#F8F9FB]"
        style={{
          WebkitMaskImage:
      ...
      {/* Content section */}
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-6 py-7 md:py-10 md:px-8">
        <div className="flex flex-col gap-[14px]">
          <svg
            className="block h-10 w-10 text-black/20"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
          ...
          >
            ...
            <div
              aria-hidden="true"
              className="invisible flex min-h-[120px] flex-col gap-[14px]"
            >
            ...
            <div
              key={displayIndex}
              className={cn(
                "absolute inset-x-0 top-0 flex flex-col gap-[14px] will-change-[transform,opacity]",
        ...
        <div className="mt-6 flex items-center gap-2 md:mt-0">
          <button
            ...
            className="grid h-8 w-8 cursor-pointer place-items-center rounded-full border border-black/15 bg-white text-black transition-[opacity,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:enabled:scale-[1.08] active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-30 shadow-2xs"
          >
            <svg
